...
- **Автоматическая настройка виртуального окружения**: Программа автоматически создаёт виртуальное окружение,
 если оно ещё не создано или пересоздаст его, если оно не подходит к установденной на компьютере версии python.
...
Предназначена для выполнения Python-скрипта с именем `main.py` из текущей папки (версия run_app_project),
или из подпапки подпапке `app` (в версии run_app_app). 

Другие файлы и папки проекта также должны находится в подпапке `app` (в версии run_app_app) либо как обычно, 
в папке проекта( в версии run_app_project). В этом случае файл утилиты должет находится так же в корневой папке проекта.
Версия утилиты и способ ее использования задается при ее компилляции. 

Можно скомпилировать в exe-файл поочередно сразу две версии и использовать их или одновременно или какая больше подходит.
...
Для коректной работы утилиты и установки всех необходимых зависимостей в запускаемом приложении  или проекте, 
требуется создать в корне запускаемого приложения-проекта  файл `requirements.txt` (см. инструкцию)

### Пример структуры проекта версии run_app_app:
